...
class Notifications extends Admin_Controller {
    ...
    public function index() { $this->render('admin/notifications/index',array('title'=>'ส่งการแจ้งเตือน','page_title'=>'ส่งการแจ้งเตือน','roles'=>$this->User_model->get_all_roles(),'employees'=>$this->User_model->get_all(array('status'=>'active'),300))); }
    public function send() {
        ...
        switch($type) {
            ...
            case 'individual': $this->Notification_model->create(array('user_id'=>$this->input->post('user_id'),'sender_id'=>$this->current_user->user_id,'type'=>$nt,'title'=>$title,'message'=>$msg,'link'=>$link)); break;
        }
