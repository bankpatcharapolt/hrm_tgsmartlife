...
<div class="row g-2 mb-3">
  <div class="col-6 col-md-3">
    <div class="stat-card">
      ...
      <div>
        <div class="s-lbl">มาทำงาน</div>
        <div class="s-val text-success"><?= $summary['present'] ?></div>
        <div class="s-sub">วัน</div>
      </div>
    </div>
  </div>
  <div class="col-6 col-md-3">
    <div class="stat-card">
      ...
      <div>
        ...
        <div class="s-val text-warning"><?= $summary['late'] ?></div>
        <div class="s-sub"><?= $summary['total_late_min'] ?> นาที</div>
      </div>
    </div>
  </div>
  <!-- <div class="col-6 col-md-3">
    <div class="stat-card">
      <div class="s-ico" style="background:#eff6ff;color:#1a56db"><i class="bi bi-calendar-check"></i></div>
      <div>
        <div class="s-lbl">ลา</div>
        <div class="s-val"><?= $summary['leave'] ?></div>
        <div class="s-sub">วัน</div>
      </div>
    </div>
  </div> -->
  <div class="col-6 col-md-3">
    <div class="stat-card">
      ...
      <div>
        <div class="s-lbl">OT</div>
        <div class="s-val"><?= number_format($summary['total_ot'], 1) ?></div>
        ...
        <tbody>
          <?php if (!empty($records)):
            foreach ($records as $r): ?>
              <tr>
                <td style="font-size:.83rem;white-space:nowrap"><?= date('d/m/Y', strtotime($r->date)) ?><br><small
                    class="text-muted"><?= date('D', strtotime($r->date)) ?></small></td>
                <td>
                  <?php if (!empty($r->shift_name)): ?>
                    <span class="badge"
                      style="background:<?= $r->shift_color ?? '#6b7280' ?>;font-size:.67rem"><?= $r->shift_name ?></span>
          ...
          <div class="col-6">
            ...
            <select name="shift_id" class="form-select form-select-sm">
              ...
              <?php foreach ($shifts as $s): ?>
                <option value="<?= $s->id ?>"><?= $s->name ?> (<?= substr($s->start_time, 0, 5) ?>–<?= substr($s->end_time, 0, 5) ?>)
                </option>
              <?php endforeach; ?>
            </select>
          </div>
            ...
            <div class="p-2 rounded" style="background:#eff6ff;border:1px solid #bae6fd">
              <div class="row g-2">
                ...
                <div class="col-4">
                  ...
                  <select name="leave_unit" class="form-select form-select-sm"
                    onchange="toggleHour(this.value,'addHour')">
                ...
                <div class="col-12" id="addHour" style="display:none">
                  <div class="row g-1">
                    <div class="col-5"><label class="form-label small">เวลาเริ่ม</label><input type="time"
                        name="leave_start_hour" class="form-control form-control-sm"></div>
                    <div class="col-2 d-flex align-items-end pb-1 justify-content-center"><span
                        class="text-muted">–</span></div>
                    <div class="col-5"><label class="form-label small">เวลาสิ้นสุด</label><input type="time"
                        name="leave_end_hour" class="form-control form-control-sm"></div>
                  </div>
                </div>
              </div>
            </div>
