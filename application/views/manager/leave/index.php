  ...
  <div class="card-body p-0">
    <div class="table-responsive">
      <table class="table table-hover mb-0">
        <thead><tr><th>พนักงาน</th><th>ประเภท</th><th>วันที่</th><th>วัน</th><th>เหตุผล</th><th>สถานะ</th><th>จัดการ</th></tr></thead>
        <tbody>
          ...
          <tr>
            ...
            <td>
              ...
              <button class="btn btn-danger btn-sm px-2 py-0 ms-1" data-bs-toggle="modal" data-bs-target="#mRj<?=$req->id?>">✗</button>
              <?php else:?><span class="text-muted small">ดำเนินการแล้ว</span><?php endif;?>
            </td>
          </tr>
          <div class="modal fade" id="mAp<?=$req->id?>" tabindex="-1"><div class="modal-dialog modal-sm"><div class="modal-content"><div class="modal-header py-2 bg-success text-white"><h6 class="modal-title mb-0">อนุมัติการลา</h6><button class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><?=form_open('manager/leave/approve/'.$req->id)?><input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash()?>"><div class="modal-body"><p class="small mb-2">อนุมัติการลาของ <strong><?=$req->first_name?></strong> จำนวน <?=$req->total_days?> วัน?</p><textarea name="note" class="form-control form-control-sm" rows="2" placeholder="หมายเหตุ (ถ้ามี)"></textarea></div><div class="modal-footer py-2"><button type="submit" class="btn btn-success btn-sm">ยืนยัน</button></div><?=form_close()?></div></div></div>
          <div class="modal fade" id="mRj<?=$req->id?>" tabindex="-1"><div class="modal-dialog modal-sm"><div class="modal-content"><div class="modal-header py-2 bg-danger text-white"><h6 class="modal-title mb-0">ปฏิเสธการลา</h6><button class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><?=form_open('manager/leave/reject/'.$req->id)?><input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash()?>"><div class="modal-body"><textarea name="note" class="form-control form-control-sm" rows="2" placeholder="เหตุผล (จำเป็น)" required></textarea></div><div class="modal-footer py-2"><button type="submit" class="btn btn-danger btn-sm">ยืนยัน</button></div><?=form_close()?></div></div></div>
          <?php endforeach;else:?><tr><td colspan="7" class="text-center text-muted py-5">ไม่มีข้อมูลการลา</td></tr><?php endif;?>
        </tbody>
      </table>
    </div>
  </div>
