...
class Notification_model extends CI_Model {
    ...
    public function send_to_all($sid,$type,$title,$msg,$link='') {
        foreach($this->db->where('status','active')->get('users')->result() as $u)
            $this->create(['user_id'=>$u->id,'sender_id'=>$sid,'type'=>$type,'title'=>$title,'message'=>$msg,'link'=>$link]);
    }
    public function send_to_role($sid,$slug,$type,$title,$msg,$link='') {
        ...
        foreach($users as $u) $this->create(['user_id'=>$u->id,'sender_id'=>$sid,'type'=>$type,'title'=>$title,'message'=>$msg,'link'=>$link]);
    }
    ...
    public function mark_read($id,$uid) { $this->db->where('id',$id)->where('user_id',$uid)->update('notifications',['is_read'=>1]); }
    public function mark_all_read($uid) { $this->db->where('user_id',$uid)->where('is_read',0)->update('notifications',['is_read'=>1]); }
}
